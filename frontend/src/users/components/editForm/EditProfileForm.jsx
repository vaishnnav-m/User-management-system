...
function EditProfileForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const token = sessionStorage.getItem("token");
      const response = await axios.post("/api/users/editProfile", formData, {
        headers: {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
      <div className="relative h-40 bg-gradient-to-r from-blue-500 to-teal-500">
        <button onClick={() => navigate("/") } className="rounded-full mx-2 my-2 bg-white relative text-black w-[50px] h-[50px]"> <i className="fa-solid fa-arrow-right-from-bracket rotate-[180deg] absolute -translate-x-1/2 -translate-y-1/2"></i> </button>
